...
class AdaptiveYOLO(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        if targets != None:
            ...
            active_classes = self.mode_classes_list[current_mode]
            print("Class to Cluster mapping is ", self.mode_dicts_class_to_cluster[current_mode])
            to_delete = []
            for i, ele in enumerate(modified_targets[:,1]):
                if ele in active_classes:
                    modified_targets[i,1] = torch.tensor(self.mode_dicts_class_to_cluster[current_mode][int(ele)], dtype=torch.float64, device=x.get_device())
                else:
                    to_delete.append(i)
...
# This one can not be train, if you want to train use Darknet model in models.py
