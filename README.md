...
## Abstract
Object Detection is one of the most important computer vision tasks that have countless real-time application. One of the biggest challenges is that those models have big size and computational complexity that is not available on embedded devices. Luckily, those tasks can be divided into simpler tasks by considering the temporal and spatial information while running those models. This repo represents a work in-progress to use the co-existance of the objects in creating a self adaptive network with samller size and number of operations.

## 1. Baseline - Object Detection using Yolo
This is the baseline, for training use 
`python3 train_baseline.py --model_def config/yolov3-tiny.cfg --data_config config/coco.data --epochs 200 --batch_size 128`
for testing run 
`python3 test_baseline.py --model_def config/yolov3-tiny.cfg --data_config config/coco.data --weights_path checkpoints/yolov3_tiny.pth --batch_size 1`
...
3. Explore more parameters for training by running `python3 train_adaptive1.py --help`
...
2. Run the training using `python3 train_branch_controller.py --model_def config/branch_controller.cfg --data_config config/coco.data --clusters_path clusters_3b.data --ckpt_prefix "bnch3" --epochs 100 --batch_size 128`

#### Testing the Adaptive Model
The model can be tested under few different conditions:

1. Get the Oracle for your model: This is the highest bound of the backbone and detection head, we do this by assuming a perfect branch controller. You can get the Oracle accuracy by setting the max_bound option `--max_bound True` in the testing command. Example: `python3 test_adaptive1.py --model_def config/yolov3_ada.cfg --data_config config/coco.data --clusters_path clusters.data --weights_path weights.pth --batch_size 1 --max_bound True`

2. To use the branch controller, you should set `--hier_class True` and you should provide the branch controller model using `--hier_model_cfg` option as well as the weights for the branch controller using the `--hier_model` option. Example `python3 test_adaptive1.py --model_def config/yolov3_ada.cfg --data_config config/coco.data --clusters_path clusters.data --weights_path weights.pth --batch_size 1 --hier_class True --hier_model_cfg config/hier_clus.cfg --hier_model branch_controller.pth`
